...
export function AppSidebar({ onLogout, open, onClose }: AppSidebarProps) {
                  ...
                  <SidebarMenuItem key={item.title}>
                    ...
                    >
                      <Link to={item.url} className="flex items-center gap-3 w-full">
                        ...
                        {item.badge && (
                          <Badge className="bg-slate-700 text-slate-200 text-xs px-2 py-0.5">{item.badge}</Badge>
                        )}
                      </Link>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                ...
                <SidebarMenuItem>
                  <SidebarMenuButton 
                    asChild 
                    isActive={location.pathname === '/admin/provider' || location.pathname === '/admin/multi-provider'}
                    style={{ backgroundColor: (location.pathname === '/admin/provider' || location.pathname === '/admin/multi-provider') ? '#dc2626' : 'transparent' }}
                    className="group w-full justify-start gap-3 rounded-xl px-4 py-3 text-slate-300 transition-all duration-200 hover:bg-slate-800/80 hover:text-white"
                  >
                    <Link to="/admin/provider" className="flex items-center gap-3 w-full">
                      <Cloud className="h-5 w-5" />
                      <span className="text-white">Multi-Provider Management</span>
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
                <SidebarMenuItem>
                  ...
                  >
                    <Link to="/admin/referral" className="flex items-center gap-3 w-full">
                      ...
                      <Badge className="bg-orange-600 text-white text-xs px-2 py-0.5">Admin</Badge>
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              </SidebarMenu>
                  ...
                  <SidebarMenuItem key={item.title}>
                    ...
                    >
                      <Link to={item.url} className="flex items-center gap-3 w-full">
                        ...
                        <span className="font-medium flex-1 text-white">{item.title}</span>
                        {item.badge && (
                          <Badge variant="destructive" className="bg-red-600 text-white text-xs px-2 py-0.5">{item.badge}</Badge>
                        )}
                      </Link>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <DropdownMenu>
              ...
              <DropdownMenuContent side="top" className="w-64" style={{ backgroundColor: '#1e293b' }}>
                <DropdownMenuItem 
  ...
  if (isMobile) {
    return (
      <>
        ...
        <div
          className={`fixed top-0 left-0 z-50 h-full w-64 bg-slate-950 shadow-lg transition-transform duration-200 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}
        >
          ...
          <div className="flex h-full flex-col text-white pt-12">
            {sidebarContent}
          </div>
        </div>
      </>
  ...
  return (
    <Sidebar className="h-screen w-64 flex-shrink-0 transition-all duration-300 z-50 top-0 left-0 relative" style={{ backgroundColor: '#020617' }}>
